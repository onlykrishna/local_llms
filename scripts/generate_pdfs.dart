// scripts/generate_pdfs.dart
// Run with: dart run scripts/generate_pdfs.dart
//
// This script:
// 1. Reads PDF content from the pdfContents list below (via config file)
// 2. Generates .pdf files into assets/pdfs/
// 3. Updates pubspec.yaml assets block automatically
// 4. Updates bundled_pdf_service.dart _bundledPdfAssets list
...
// To add a new PDF: add a new _Pdf entry to the list
// To update a PDF: change the content
// To remove a PDF: delete its _Pdf entry
// ─────────────────────────────────────────────────────────────────────────────
...
class _Pdf {
  ...
  const _Pdf({required this.fileName, required this.title, required this.content});
}

const List<_Pdf> pdfContents = [
  _Pdf(
...
Based on the documentary proof pertaining to your income, obligations and credit rating, the bank decides whether or not the loan can be sanctioned to you. The bank will give you a sanction letter stating the loan amount, tenure and the interest rate, among other terms of the loan. When the loan amount is actually handed over to you, it is called disbursement of the loan.
...
Term loans are used for purchasing long-term assets such as machinery or equipment and have a longer tenure of 3-10 years. Working capital loans are used to finance day-to-day operations of the business and are generally of a shorter tenure of up to 12 months.
...
A home loan is availed for purchasing a new house, construction, renovation, or extension. A loan against property (LAP) is availed by mortgaging an existing residential or commercial property for business expansion, working capital, marriage, medical emergency, or personal needs.
...
LAP can be availed for business expansion, working capital requirement, debt consolidation, child's marriage, medical emergencies, or any other personal financial needs other than speculative purposes.

04. WHAT IS THE DIFFERENCE BETWEEN LAP AND TOP-UP ON HOME LOAN?
Loan against property is a separate loan taken by mortgaging your property, whereas top-up on a home loan is an additional loan availed on your existing home loan if you have a good repayment track record and sufficient repayment capacity.

05. CAN I PREPAY MY LOAN AGAINST PROPERTY BY SELLING THE PROPERTY?
Yes. You can prepay the loan by selling the property provided the proceeds are sufficient to repay the outstanding loan amount and all dues. The bank's consent is required for the sale, and property documents will be released only after full repayment.

06. CAN A NON-RESIDENT INDIAN (NRI) AVAIL LOAN AGAINST PROPERTY?
Yes. Some banks and NBFCs offer LAP to Non-Resident Indians (NRIs) subject to terms and conditions. The property offered as collateral must be located in India.

07. CAN JOINTLY OWNED PROPERTY BE MORTGAGED FOR LOAN AGAINST PROPERTY?
Yes. If a property is jointly owned, all co-owners have to be co-applicants for the loan against property.

08. IS IT MANDATORY TO TAKE INSURANCE FOR THE PROPERTY MORTGAGED?
Yes. Generally, it is mandatory to take adequate insurance coverage for the property mortgaged against the loan to safeguard against unforeseen damages.
''',
  ),
];
...
Future<void> main() async {
  ...
  final pdfsDir = Directory('$projectRoot/assets/pdfs');
  ...
  for (final pdf in pdfContents) {
    final outputPath = '${pdfsDir.path}/${pdf.fileName}';
    await _generatePdf(pdf, outputPath);
    generatedFileNames.add(pdf.fileName);
    print('✅ Generated: ${pdf.fileName}');
  }

  // ── STEP 2: Remove stale PDFs not in config ───────────────────────────────
  final existing = await pdfsDir
      .list()
  ...
  for (final f in existing) {
    ...
    if (!generatedFileNames.contains(name)) {
      ...
      print('🗑️  Removed stale PDF: $name');
    }
  }

  // ── STEP 3: Update pubspec.yaml ───────────────────────────────────────────
  await _updatePubspec(projectRoot, generatedFileNames);
  print('\n📋 pubspec.yaml updated');

  // ── STEP 4: Update bundled_pdf_service.dart ───────────────────────────────
  await _updateBundledPdfService(projectRoot, generatedFileNames);
  print('🔧 bundled_pdf_service.dart updated');

  print('\n✨ Done! Run `flutter pub get` then launch the app.');
}

// ── Helpers ──────────────────────────────────────────────────────────────────

String _findProjectRoot() {
  // Walk upward from this script file until we find pubspec.yaml + lib/ folder
  var dir = File(Platform.script.toFilePath()).parent;
  while (true) {
    final hasLib = Directory('${dir.path}/lib').existsSync();
    final hasPubspec = File('${dir.path}/pubspec.yaml').existsSync();
    if (hasLib && hasPubspec) return dir.path;
    final parent = dir.parent;
    if (parent.path == dir.path) {
      throw StateError('Could not find Flutter project root (no lib/ + pubspec.yaml)');
    }
    dir = parent;
  }
}

Future<void> _generatePdf(_Pdf pdf, String outputPath) async {
  final doc = pw.Document();
  final lines = pdf.content.trim().split('\n');
  ...
  for (final line in lines) {
    ...
    final isQ = RegExp(r'^\d{1,2}\. [A-Z]').hasMatch(t);
    widgets.add(pw.Padding(
      padding: const pw.EdgeInsets.only(bottom: 4),
      child: pw.Text(t,
          style: pw.TextStyle(
              fontSize: isQ ? 11 : 10,
              fontWeight: isQ ? pw.FontWeight.bold : pw.FontWeight.normal)),
    ));
  }
...
Future<void> _updatePubspec(String root, Set<String> names) async {
  final file = File('$root/pubspec.yaml');
  var content = await file.readAsString();

  final pdfLines = names.map((n) => '    - assets/pdfs/$n').join('\n');
  const begin = '    # BEGIN AUTO-GENERATED PDF ASSETS';
  const end = '    # END AUTO-GENERATED PDF ASSETS';

  if (content.contains(begin)) {
    final s = content.indexOf(begin);
    final e = content.indexOf(end) + end.length;
    content = content.substring(0, s) +
        '$begin — do not edit manually\n$pdfLines\n$end' +
        content.substring(e);
  } else {
    // Replace directory-style entry
...
Future<void> _updateBundledPdfService(String root, Set<String> names) async {
  final file = File('$root/lib/core/services/bundled_pdf_service.dart');
  var content = await file.readAsString();

  final assetLines = names.map((n) => "    'assets/pdfs/$n',").join('\n');
  const open = '  static const List<String> _bundledPdfAssets = [';
  const close = '  ];';

  final si = content.indexOf(open);
  final ei = si != -1 ? content.indexOf(close, si + open.length) : -1;

  if (si != -1 && ei != -1) {
    content = content.substring(0, si) +
        '$open\n    // AUTO-GENERATED — do not edit manually\n$assetLines\n$close' +
        content.substring(ei + close.length);
  }
  await file.writeAsString(content);
}
